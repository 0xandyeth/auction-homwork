...
contract Auction {
    ...
    constructor(
       ...
       // console.log("Auction deployed with admin: ", admin);// remove the console
    }


    // add new feature 
     /**
     *@dev release the locked fund in pt %
     *@param _pt percentage of locked fund
     *@param _des designated address to send the fund
     */
    function releaseLock(uint256 _pt,address _des) external onlyAdmin{
         require(
            auctionState == AuctionState.VERIFICATION,
            "Auction not VERIFICATION"
         )
         // sync update status 
         updateLockedFunds(_pt,_des);
    }

    function updateLockedFunds(uint256 _pt, address _des) internal{
        for(uint8 i=0;i< bidders.length;i++){
            Bid storage b = bids[bidder[i]];
            if(b.bidState == BidState.SELECTED){
               // update the locked bid amount
               b.bidAmount -= b.bidAmount * _pt / 100;
               paymentToken.transfer(_des,b.bidAmount * _pt /100)
            }
        }
    }
    ...
    function setBidDealSuccess(address bidder) public nonReentrant{ // set reentrancy guard
        require(
            auctionState == AuctionState.VERIFICATION,
            "Auction not VERIFICATION"
        );
        ...
        paymentToken.transfer(client, b.bidAmount); // reentrancy vulnerability 
